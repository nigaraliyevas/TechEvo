...
import { useGetCreditCardQuery, useGetCreditHeader2Query, useGetCreditHeader1Query } from "../../../redux/sercives/serviceApi";

const Credit = () => {
  const { data: creditHeader1, error: creditHeader1Error, isLoading: creditHeader1Loading } = useGetCreditHeader1Query();
  ...
  return (
    <div className={styles.creditCont}>
      <div className={styles.crHeader}>
        ...
        <div className={styles.headInfo}>{creditHeader1?.headerDescription}</div>
      </div>

      <div className={styles.myCont}>
        <div className={styles.crMid}>
          <div className={styles.midTitle}>{creditHeader2?.headerName}</div>
          <div className={styles.midInfo}>{creditHeader2?.headerDescription}</div>
        </div>
      </div>
      ...
      <div className={styles.crEnd}>
        <div className={`${styles.myCont} ${styles.endCont}`}>
          <div className={styles.endLeft}>
            ...
            <div className={styles.endInfo}>{credit?.cardDescription}</div>
          </div>
